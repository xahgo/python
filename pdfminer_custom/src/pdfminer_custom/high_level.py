...
from .MyPDFFont import MyPDFFont
...
# ) -> dict[int, dict[int, MyPDFFontObj]]:
    """Parse and return the text contained in a PDF file.
    ...
    with open_filename(pdf_file, "rb") as fp, StringIO() as output_string:
        ...
        device = TextConverter(rsrcmgr, output_string, codec=codec, laparams=laparams)
        interpreter = PDFPageInterpreter(rsrcmgr, device)
        # dict[pageno, dict[objid, fontobj]]
        # dic_pageno_fontobjs_in_page: dict[int, dict[int, MyPDFFontObj]] = {}
        my_pdf_font = MyPDFFont()
        for pageno, page in enumerate(PDFPage.get_pages(
            ...
            # # [objid, fontobj]
            # dic_fontobj: dict[int, MyPDFFontObj] = {}
            # extract fonts. 페이지별로 사용되는 폰트들의 정보를 모두 추출.
            interpreter.extract_page_font(page, pageno + 1, my_pdf_font)
            # add font
            # dic_pageno_fontobjs_in_page[pageno + 1] = dic_fontobj
        # my_pdf_font = MyPDFFont(MyPDFFontData.dic_fontdata, MyPDFFont.dic_cidary, MyPDFFont.dic_pageno_fontobjs_in_page)

        # # make fontmap
        # dic_fontmap: dict[str, MyPDFFontData] = {}
        # for key_fontid, pdf_font in interpreter.fontmap.items():
        #     for key_objid, dic_fontobj in dic_pageno_fontobjs_in_page.items():
        #         my_fonts = [v for v in dic_fontobj.values() if v.fontname == pdf_font.fontname]
        #         if len(my_fonts) == 0:
        #             continue
        #         if len(my_fonts) > 1:
        #             if my_fonts[0] != my_fonts[1]:
        #                 # fontmap에 있는건 cid2unicode가 달라도 폰트 이름만 같으면 같은걸로 치는듯.
        #                 pass
        #         fontid = cast(str, key_fontid)
        #         dic_fontmap[fontid] = my_fonts[0]
        #         # fontname과 basefont가 다른경우가 있는지 확인
        #         diff_names = [font for font in dic_fontobj.values() if font.fontname != font.basefont]
        #         if len(diff_names) > 1:
        #             pass
