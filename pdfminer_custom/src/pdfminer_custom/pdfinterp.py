...
from .MyPDFFont import MyPDFFontData, MyPDFFont
from pdfminer_custom import settings
...
class PDFPageInterpreter:
    ...
    def extract_page_font(self,
                          page: PDFPage,
                          pageno: int,
                          my_font: MyPDFFont,
                          # dic_font_out: dict[int, MyPDFFontObj]
    ) -> None:
        log.debug("extract_page_font: %r", page)
        (x0, y0, x1, y1) = page.mediabox
        if page.rotate == 90:
            ctm = (0, -1, 1, 0, -y0, x1)
        elif page.rotate == 180:
            ...
            ctm = (1, 0, 0, 1, -x0, -y0)
        self.device.begin_page(page, ctm)
        self.extract_contents_font(page.resources, page.contents, pageno, my_font)
        self.device.end_page(page)


    def extract_contents_font(
        self,
        ...
        """
        log.debug("extract_contents_font: resources=%r, streams=%r",resources,streams)
        self.init_resources(resources)
        # cached_font
        for key_objid, pdf_font in self.rsrcmgr._cached_fonts.items():
            # fontdata  = MyPDFFontData()
            if isinstance(pdf_font, PDFType1Font):
                pdf_font = cast(PDFType1Font, pdf_font)
                type = 'Type1'
            elif isinstance(pdf_font, PDFType3Font):
                pdf_font = cast(PDFType3Font, pdf_font)
                type = 'Type3'
            elif isinstance(pdf_font, PDFTrueTypeFont):
                pdf_font = cast(PDFTrueTypeFont, pdf_font)
                type = 'TrueType'
            elif isinstance(pdf_font, PDFCIDFont):
                pdf_font = cast(PDFCIDFont, pdf_font)
                type = 'CID'
            else:
                type = 'unknown' # Type0
            ...
            if type == 'unknown':
                ...
                basefont = pdf_font.basefont
            fontname = pdf_font.fontname
            # fontdata를 캐쉬에 쌓음.
            fontdata = MyPDFFontData.add(type, font_id, basefont, fontname)
            # cid2unicide를 캐쉬에 쌓음.
            if hasattr(pdf_font, "cid2unicode"):
                cid_id = my_font.add_cidary(pdf_font.cid2unicode)
                # object형을 int로 캐스트
                objid = cast(int, key_objid) if key_objid != None else 0
                # font_obj = MyPDFFontObj(objid, fontdata, cid_id)
                # dic_font_out[objid] = font_obj
                my_font.add_used_font(pageno, objid, fontdata, cid_id)
            else:
                pass
        # self.init_state(ctm)
